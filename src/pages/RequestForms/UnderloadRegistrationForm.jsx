...
const UnderloadRegistrationForm = () => {
  ...
  // Consideration Data States (ข้อมูลประกอบการพิจารณา)
  const [accumulatedCredits, setAccumulatedCredits] = useState('');
  const [gpa, setGpa] = useState('');
  const [reasonCategory, setReasonCategory] = useState('coop'); // ตั้งค่าเริ่มต้นเป็น สหกิจศึกษา
  const [otherReasonText, setOtherReasonText] = useState('');
  
  // Past History States (ประวัติการลงต่ำกว่าเกณฑ์)
  const [pastUnderload, setPastUnderload] = useState('never'); 
  const [pastUnderloadTerm, setPastUnderloadTerm] = useState('1');
  const [pastUnderloadYear, setPastUnderloadYear] = useState('');
  ...
  const fetchExistingSubmission = async (id) => {
    try {
      ...
      // Map reason category
      if (data.reason_category === 'สหกิจศึกษาหรือฝึกประสบการณ์ภาคสนาม') setReasonCategory('coop');
      else if (data.reason_category === 'เป็นภาคการศึกษาสุดท้ายที่จะสำเร็จการศึกษาตามหลักสูตร') setReasonCategory('last_semester');
      else if (data.reason_category === 'กรณีเจ็บป่วย (แนบใบรับรองแพทย์)') setReasonCategory('sick');
      else setReasonCategory('other');
      
      setOtherReasonText(data.other_reason_text || '');
      
      setPastUnderload(data.past_underload_status === 'เคยลงทะเบียนต่ำกว่าเกณฑ์' ? 'ever' : 'never');
      setPastUnderloadTerm(data.past_underload_term || '1');
      setPastUnderloadYear(data.past_underload_year || '');
    } catch {
      setIsEditMode(false);
  ...
  const handleSubmit = async (e) => {
    ...
    // แปลง Category ให้เป็นข้อความภาษาไทยเพื่อเก็บลง Database
    let categoryText = '';
    if (reasonCategory === 'coop') categoryText = 'สหกิจศึกษาหรือฝึกประสบการณ์ภาคสนาม';
    else if (reasonCategory === 'last_semester') categoryText = 'เป็นภาคการศึกษาสุดท้ายที่จะสำเร็จการศึกษาตามหลักสูตร';
    else if (reasonCategory === 'sick') categoryText = 'กรณีเจ็บป่วย (แนบใบรับรองแพทย์)';
    else categoryText = 'อื่นๆ';

    // Prepare data object
    const formData = {
      subject: "ขอลงทะเบียนเรียนต่ำกว่าหน่วยกิตที่กำหนด",
      ...
      gpa: gpa,
      reason_category: categoryText,
      other_reason_text: reasonCategory === 'other' ? otherReasonText : '',
      past_underload_status: pastUnderload === 'ever' ? 'เคยลงทะเบียนต่ำกว่าเกณฑ์' : 'ไม่เคยลงทะเบียนต่ำกว่าเกณฑ์',
      past_underload_term: pastUnderload === 'ever' ? pastUnderloadTerm : '',
      past_underload_year: pastUnderload === 'ever' ? pastUnderloadYear : '',
    };

    if (!window.confirm("คุณยืนยันที่จะส่งคำร้องขอลงทะเบียนเรียนต่ำกว่าหน่วยกิตใช่หรือไม่?")) return;
      ...
      if (isEditMode && submissionId) {
        ...
        response = await fetch('/student/api/submissions', {
          ...
          body: JSON.stringify({
            ...
            form_id: 6, // form_id = 6 สำหรับลงทะเบียนต่ำกว่าเกณฑ์
            form_data: formData,
          }),
        });
      }
  ...
  return (
    <div className="max-w-5xl mx-auto p-8 bg-white shadow-xl rounded-md my-10 border-t-8 border-indigo-700">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-800">ใบคำร้องขอลงทะเบียนเรียนต่ำกว่าหน่วยกิตที่กำหนด</h2>
      </div>
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
          <div className="flex items-center gap-2">
            ...
            <span>ขอลงทะเบียนเรียนต่ำกว่าหน่วยกิตที่กำหนด</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm space-y-6">
          <div className="flex flex-wrap items-center gap-2">
            <span>มีความประสงค์ขอลงทะเบียนเรียนต่ำกว่าหน่วยกิตที่กำหนด ในภาคการศึกษาที่</span>
          ...
          <div className="flex flex-col gap-2">
            <label className="font-semibold text-gray-700">เนื่องจาก (ระบุเหตุผลเบื้องต้น):</label>
        ...
        <div className="p-6 bg-indigo-50 border border-indigo-200 rounded-lg shadow-sm space-y-4">
          ...
          {/* Condition Category (ปรับเงื่อนไขใหม่ตามที่บรีฟ) */}
          <div className="mt-4">
            <label className="font-semibold text-gray-700 block mb-2">เงื่อนไขการขอลงทะเบียน:</label>
            <div className="flex flex-col gap-3 pl-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="reasonCategory" value="coop" checked={reasonCategory === 'coop'} onChange={(e) => setReasonCategory(e.target.value)} className="text-indigo-600" />
                <span>สหกิจศึกษาหรือฝึกประสบการณ์ภาคสนาม</span>
              </label>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="reasonCategory" value="sick" checked={reasonCategory === 'sick'} onChange={(e) => setReasonCategory(e.target.value)} className="text-indigo-600" />
                <span>กรณีเจ็บป่วย (แนบใบรับรองแพทย์)</span>
              </label>
              ...
              {reasonCategory === 'other' && (
                <div className="pl-6 mt-1">
                  <input 
                    ...
                    placeholder="โปรดระบุเงื่อนไขเพิ่มเติม..." 
          ...
          <div className="mt-4 border-t border-indigo-200 pt-4">
            <label className="font-semibold text-gray-700 block mb-2">ประวัติการขอลงทะเบียนเรียนต่ำกว่าหน่วยกิตที่กำหนด:</label>
            <div className="flex flex-col gap-3 pl-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="pastUnderload" value="never" checked={pastUnderload === 'never'} onChange={(e) => setPastUnderload(e.target.value)} className="text-indigo-600" />
                <span>ไม่เคยขอลงทะเบียนเรียนต่ำกว่าเกณฑ์</span>
              </label>

              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="pastUnderload" value="ever" checked={pastUnderload === 'ever'} onChange={(e) => setPastUnderload(e.target.value)} className="text-indigo-600" />
                <span>เคยขอลงทะเบียนเรียนต่ำกว่าเกณฑ์มาแล้ว</span>
              </label>
              ...
              {pastUnderload === 'ever' && (
                <div className="pl-6 flex items-center gap-2 mt-1">
                  ...
                  <select 
                    value={pastUnderloadTerm} 
                    onChange={(e) => setPastUnderloadTerm(e.target.value)} 
                    className="border-b border-gray-400 bg-transparent outline-none px-1 text-center"
                    required={pastUnderload === 'ever'}
                  >
                  ...
                  <input 
                    type="text" 
                    value={pastUnderloadYear} 
                    onChange={(e) => setPastUnderloadYear(e.target.value)} 
