...
const LateRegistrationForm = () => {
  ...
  const [studentType, setStudentType] = useState('ปกติ'); // เพิ่ม State นักศึกษาภาค
  const [yearOfStudy, setYearOfStudy] = useState('1'); // เพิ่ม State ชั้นปี
  ...
  const handleSubmit = async (e) => {
    ...
    // คัดกรองเอาเฉพาะรายวิชาที่นักศึกษากรอกรหัสวิชาจริงๆ (ตัดบรรทัดว่างทิ้ง)
    const filledCourses = courses.filter(c => c.courseCode.trim() !== '');

    if (filledCourses.length === 0) {
        alert('กรุณากรอกรายวิชาที่ต้องการลงทะเบียนล่าช้าอย่างน้อย 1 วิชานะคะ');
        return;
    }

    if (!window.confirm("คุณยืนยันที่จะส่งคำร้องขอลงทะเบียนเรียนล่าช้าใช่หรือไม่? (หากอนุมัติ จะมีค่าปรับในภายหลัง)")) return;

    // รวบรวมข้อมูลทั้งหมดให้อยู่ในรูปแบบ JSON
    const formData = {
      subject: "ขอลงทะเบียนเรียนล่าช้า",
      student_type: studentType, // ส่งนักศึกษาภาค
      year_of_study: yearOfStudy, // ส่งชั้นปี
      term: term,
    ...
    try {
      const response = await fetch('/student/api/submissions', {
        ...
        body: JSON.stringify({
          ...
          form_id: 3, // ส่งไป form_id 3 (ลงทะเบียนล่าช้า)
          form_data: formData
        }),
      });
  ...
  return (
    <div className="max-w-5xl mx-auto p-8 bg-white shadow-xl rounded-md my-10 border-t-8 border-orange-600">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-800">ใบคำร้องขอลงทะเบียนเรียนล่าช้า</h2>
      </div>
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
          <div className="flex items-center gap-2">
            ...
            <span>ขอลงทะเบียนเรียนล่าช้า</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        {/* ข้อมูลส่วนตัว (อัปเกรดให้เหมือนฟอร์มลงเกิน) */}
        ...
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
          <div className="flex flex-wrap items-center gap-2 mb-6 text-gray-800">
            <span>มีความประสงค์ขอลงทะเบียนเรียนล่าช้า ในภาคการศึกษาที่</span>
          ...
          <div className="flex flex-col gap-2 mb-6">
            <label className="font-semibold text-gray-700">เนื่องจาก (ระบุเหตุผลความจำเป็นที่ล่าช้า):</label>
          ...
          <p className="font-semibold mb-2 text-gray-800">โดยมีรายวิชาที่ต้องการขอลงทะเบียน ดังนี้:</p>
