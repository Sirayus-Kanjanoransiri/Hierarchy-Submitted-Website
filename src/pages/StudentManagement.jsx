    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 font-['Inter']">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 flex items-center justify-between">
            <h1 className="text-3xl font-bold text-gray-900">จัดการข้อมูลนักศึกษา (Students)</h1>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-8 border-t-4 border-blue-500">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">สาขาวิชา (Department) *</label>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">อาจารย์ที่ปรึกษา (Advisor)</label>
                    <select name="advisor_id" value={formData.advisor_id} onChange={handleChange}
                        className="w-full p-2 border rounded-md focus:ring-blue-500 focus:border-blue-500 bg-white">
                        <option value="">-- เลือกอาจารย์ที่ปรึกษา --</option>
                        {approvers.map(a => (
                            <option key={a.id} value={a.id}>{a.approver_prefix}{a.full_name}</option>
                        ))}
                    </select>
                </div>
            </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {students.length > 0 ? (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                        ${item.status === '1' ? 'bg-green-100 text-green-800' : 
                          item.status === '2' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-4 text-center text-sm text-gray-500">ไม่พบข้อมูลนักศึกษา</td>
                </tr>
              )}
            </tbody>
          </table>
